...
for(i in patient){
  ...
  kpPoints(kp, chr = meth_data$chromosome_name, 
           r0 = 0, r1 = 0.23, 
           x = meth_data$end_position, y = (meth_data[ ,i]), 
           col = meth_color, cex = cx)
  ...
  kpBars(kp, chr = gene_data$chromosome_name, 
         ...
         y1 = (gene_data[ ,i] / max(gene_data[ ,i])), border = "slateblue4", cex = 1, lwd = 2)
  ...
  kpBars(kp, chr = gene_data$chromosome_name, 
         r0 = 0.75, r1 = 1, 
         x0 = gene_data$start_position, x1 = gene_data$end_position, 
         y1 = (gene_data[ ,i] / max(gene_data[ ,i])), border = "slateblue4", lwd = 2)
  kpSegments(kp, chr = var_data$chromosome_name,
             r0 = 0.75, r1 = 1,
             x0 = var_data$start_position,
             x1=var_data$start_position,
...
for(i in seq(1, length(patient), by = 2)){
  ...
  kpPoints(kp, chr = meth_data$chromosome_name, r0 = 0, r1 = 0.23, x = meth_data$end_position, y = ((data_to_visualize / 2) + 0.5), col = col_to_apply, cex = cx)
  kpPoints(kp, chr = meth_data$chromosome_name, r0 = 0.75, r1 = 1, x = meth_data$end_position, y = ((data_to_visualize / 2) + 0.5), col = col_to_apply, cex = cx)
  kpAddLabels(kp, labels = "DNAMethylation", side = "left", r0 = 0, r1 = 0.23, srt = 90, label.margin = 0.03)
  ...
  Y0 = min(data_to_visualize)
  Y1 = max(data_to_visualize)
  ...
  if(Y0 == Y1){
    if(Y0 > 0){
      ...
      Y1 = (-1)*Y0
      data_to_visualize = 0.5
      from = 0
    }
  } else {
    from = 0
    data_to_visualize = data_to_visualize + abs(Y0)
    data_to_visualize = data_to_visualize / max(data_to_visualize)
  }
  
  kpAxis(kp, r0 = 0.25, r1 = 0.48, ymin = Y0, ymax = Y1)
  kpAxis(kp, r0 = 0.75, r1 = 1, ymin = Y0, ymax = Y1)
  
  kpRect(kp, chr = gene_data$chromosome_name,
         r0 = 0.25, r1 = 0.48,
         x0 = gene_data$start_position, x1 = gene_data$end_position,
         y0 = max(0, from), y1 = data_to_visualize, border = col_to_apply, lwd = 2)
  kpRect(kp, chr = gene_data$chromosome_name,
         r0 = 0.75, r1 = 1,
         x0 = gene_data$start_position, x1 = gene_data$end_position,
         y0 = max(0, from), y1 = data_to_visualize, border = col_to_apply, lwd = 2)
